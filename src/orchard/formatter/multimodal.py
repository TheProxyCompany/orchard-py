...
) -> tuple[list[dict[str, Any]], list[bytes], list[CapabilityInput], list[tuple[str, int]]]:
...
def build_multimodal_layout(
    ...
    image_matches = list(re.finditer(re.escape(placeholder_token), prompt_text)) if image_buffers else []
    ...
    if use_coord_placeholders:
        ...
        all_placeholders: list[tuple[int, int, str, int]] = []  # (start, end, type, idx)
        ...
        for start, end, ptype, idx in all_placeholders:
            ...
            if ptype == "image":
                ...
                layout.append({"type": "capability", "name": cap.name, "length": len(cap.payload)})
                coord_cap_idx += 1
        ...
        for content_type, _ in content_order:
            if content_type == "image":
                ...
                if segment_bytes:
                    layout.append({"type": "text", "length": len(segment_bytes)})
                # Add the image
                layout.append({"type": "image", "length": len(image_buffers[image_idx])})
                cursor = match.end()
                image_idx += 1
