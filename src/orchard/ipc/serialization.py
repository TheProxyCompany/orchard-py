...
def _encode_layout(
    ...
    layout_text_bytes = sum(length for seg_type, length in segments if seg_type == _SEGMENT_TYPE_TEXT)
    layout_image_bytes = sum(length for seg_type, length in segments if seg_type == _SEGMENT_TYPE_IMAGE)
...
def _build_request_payload(
    *,
    ...
    for index, prompt in enumerate(prompt_payloads):
        ...
        capability_data_offset, capability_data_size = reserve_blob(capability_data_bytes)
