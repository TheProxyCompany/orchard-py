...
async def _generate_flux_image(client: Client, prompt: str) -> ModalArtifact:
    artifacts = await client.images.agenerate(
        FLUX2_MODEL,
        prompt,
        height=512,
    ...
    artifact = artifacts[0]
    assert artifact.mime_type == "image/png"
    assert artifact.decoder_id == "flux"
    assert artifact.data.startswith(b"\x89PNG\r\n\x1a\n")
    assert len(artifact.data) > 1024
    return artifact
...
async def test_image_tool_self_loop_and_blind_verifier_flux(client: Client):
    print(
        "\n\033[1;33m━━━ gemma4 · flux2 image tool → gemma/moondream vision ━━━\033[0m",
        flush=True,
    )
    ...
    assert verifier["content"] == verifier["content_done"]
    assert "apple" in verifier["content_done"].lower(), (
        f"moondream3 did not identify the Flux-generated image: {verifier['content_done']!r}"
    )
...
async def test_image_edit_tool_blind_verifier_flux(client: Client):
    print(
        "\n\033[1;33m━━━ gemma4 · flux2 → qwen image edit → moondream vision ━━━\033[0m",
        flush=True,
    )
    ...
    for term in ("blue", "circle", "red", "square"):
        assert term in answer, (
            f"moondream3 did not identify the edited Flux source image as blue circle/red square: "
            f"{verifier['content_done']!r}"
        )
