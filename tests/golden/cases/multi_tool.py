...
async def test_multi_tool(client: Client, model: Model):
    ...
    """
    assert model.tools, "multi-tool was admitted for a model without tool support"
    ...
    for item in opened:
        assert item.name in {"get_weather", "get_time"}, (
            f"turn1: unexpected tool {item.name!r}"
        )
    ...
    if "get_weather" in by_name:
        assert json.loads(by_name["get_weather"].arguments) == {
    ...
    if remaining:
        ...
        assert turn2["added"]["function_call"] >= 1, (
            "turn2: expected the remaining tool call"
        )
        assert turn2["counts"].get("response.output_text.done", 0) == 0, (
        ...
        for call in turn2["function_calls"]:
            assert call.status == OutputStatus.COMPLETED
            assert call.name in {t["name"] for t in remaining}, (
                f"turn2: unexpected tool {call.name!r}"
            )
        ...
        assert json.loads(second["get_weather"].arguments) == {
            "location": "San Francisco"
        }
        ...
        await render_prompt_blue(
            client, model.checkpoint, prev_gen=gen2, **turn3_request
        )
        ...
        if reasoning_blocks:
            ...
            assert turn3["reasoning"].strip() == turn3["reasoning_done"], (
                "turn3: reasoning deltas != reasoning.done"
            )
        else:
            assert turn3["counts"].get("response.reasoning.delta", 0) == 0
        assert turn3["counts"].get("response.function_call_arguments.done", 0) == 0, (
            "turn3: unexpected tool call"
        )
        assert turn3["counts"]["response.output_text.done"] == 1, (
            "turn3: expected one message"
        )
        assert turn3["content"] == turn3["content_done"], (
            "turn3: content deltas != output_text.done"
        )
        final = turn3
        turns.append(turn3)
    else:
        ...
        assert by_name.keys() == {"get_weather", "get_time"}, (
    ...
    msg_open = [
        item for item in final["items_added"] if isinstance(item, OutputMessage)
    ]
