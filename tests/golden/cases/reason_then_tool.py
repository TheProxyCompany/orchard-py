...
async def test_reason_then_tool(client: Client, model: Model):
    ...
    """
    assert model.tools, "reason-then-tool was admitted without tool support"
    ...
    if turn1_reasoning_blocks:
        assert turn1_reasoning_blocks == 1, (
    ...
    if turn2_reasoning_blocks:
        assert turn2_reasoning_blocks == 1, (
            "turn2: expected exactly one reasoning block"
        )
    ...
    assert "65" in answer or "fog" in answer, (
        f"{model.template_type}: answer ignored the tool result: {answer!r}"
    )
