...
async def test_thinking_on_off(client: Client, model: Model):
    ...
    assert model.thinking is True, (
        "thinking toggle was admitted for a model that cannot toggle thinking"
    )
    print(
    ...
    # These are independent views of the same prompt. Submit both in phase 0;
    # only their assertions and golden labels differ.
    on_request = dict(
        input=conversation,
        deterministic=True,
    ...
    await asyncio.gather(
        render_prompt_blue(client, model.checkpoint, **on_request),
        render_prompt_blue(client, model.checkpoint, **off_request),
    )

    async def run_request(options):
        stream = await client.aresponses(
            model.checkpoint, stream=True, stream_tokens=True, **options
        )
        return await drain_stream(stream)

    on, off = await asyncio.gather(run_request(on_request), run_request(off_request))
    ...
    assert on["counts"]["response.reasoning.delta"] >= 1, (
        "on: reasoning produced no deltas"
    )
    assert on["reasoning"].strip() == on["reasoning_done"], (
        "on: reasoning deltas != reasoning.done"
    )
    assert "<|" not in on["reasoning"] and "</" not in on["reasoning"], (
        "on: control leak in reasoning"
    )

    on_reasoning = [
        item for item in on["items_added"] if isinstance(item, OutputReasoning)
    ]
    assert len(on_reasoning) == 1, "on: expected one reasoning item opened"
    assert on_reasoning[0].status == OutputStatus.IN_PROGRESS
    assert _reasoning_tokens(on) > 0, (
    ...
    assert off["added"].get("reasoning", 0) == 0, (
        "off: thinking still produced a reasoning block"
    )
    assert off["counts"].get("response.reasoning.delta", 0) == 0, (
        "off: reasoning deltas leaked"
    )
    assert off["counts"].get("response.reasoning.done", 0) == 0, (
        "off: reasoning.done leaked"
    )
    assert not [
        item for item in off["items_added"] if isinstance(item, OutputReasoning)
    ...
    assert off["content"] == off["content_done"], (
        "off: content deltas != output_text.done"
    )
