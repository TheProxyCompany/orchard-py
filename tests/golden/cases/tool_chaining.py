...
async def test_tool_chaining(client: Client, model: Model):
    ...
    """
    assert model.tools, "tool chaining was admitted for a model without tool support"
    ...
    assert find.name == "find_key", (
        f"turn1: must call find_key first, got {find.name!r}"
    )
    ...
    assert "response.output_text.delta" not in turn2["counts"], (
    ...
    assert unlock.name == "unlock_chest", (
        f"turn2: must call unlock_chest, got {unlock.name!r}"
    )
    ...
    conversation += [
        ...
        },
        {
            "type": "function_call_output",
            "call_id": unlock.call_id,
            "output": json.dumps({"contents": CHEST_CONTENTS}),
        },
    ]
    turn3_request = dict(
        input=conversation,
    ...
    msg_open = [
        item for item in turn3["items_added"] if isinstance(item, OutputMessage)
    ]
